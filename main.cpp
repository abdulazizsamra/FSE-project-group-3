...
int main() {
    std::cerr << "Enter the number of vertices and edges: ";
    int n, m;
    std::cin >> n >> m;
    while(true)
    {
    ...
    std::cerr << std::endl;
    std::cerr << "Enter the graph in format parent_node child_node." << std::endl;
    ...
    for (int i = 0; i < m; ++i) {
        ...
        while(true)
        {
            if(std::cin.fail())
            {
                ...
                std::cerr << "Reenter parent_node child_node: ";
                std::cin >> parent >> child;
